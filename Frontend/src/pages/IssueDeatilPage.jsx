...
function IssueDetailPage() {
  ...
  const fetchIssue = async () => {
    ...
    try {
      ...
      // Check if current user/anon already voted
      const anonId = getAnonId();
      const alreadyVoted =
        data.issue.anonymousVotes?.includes(anonId) ||
        (user && data.issue.upvotedBy?.some((u) => u._id === user._id || u === user._id));
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-6">
        <div className="text-center">
          ...
          <h2 className="text-xl font-bold text-[#0f1923] mb-2">Issue not found</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className={`ml-auto text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border ${STATUS_COLORS[issue.status]}`}>
                  {issue.status}
                </span>
              </div>

              <h1 className="text-2xl font-extrabold text-[#0f1923] mb-3" style={{ fontFamily: "Sora, sans-serif" }}>
                {issue.title}
              </h1>
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6 text-center">
              ...
              <div className="text-4xl font-extrabold text-[#0f1923] mb-1" style={{ fontFamily: "Sora, sans-serif" }}>
                {issue.votes}
              </div>
              <p className="text-xs text-gray-500 mb-4">people upvoted this</p>
                 <button
            ...
            {/* Status Timeline */}
            <div className="bg-white rounded-2xl border border-gray-200 p-6">
              <h3 className="text-sm font-bold text-[#0f1923] mb-4">Issue Timeline</h3>
                  ...
                  return (
                    <div key={step} className="flex items-center gap-3">
                      <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold transition-all
                        ${isCurrent ? "bg-[#1a56db] text-white ring-4 ring-blue-100"
                          : isDone ? "bg-[#1a56db] text-white"
                      ...
                      <div className="flex-1">
                        <p className={`text-xs font-semibold ${isDone ? "text-[#0f1923]" : "text-gray-400"}`}>
                          {step}
                        </p>
                      </div>
                      {isCurrent && (
            ...
            {/* Issue Details Card */}
            <div className="bg-white rounded-2xl border border-gray-200 p-6">
              <h3 className="text-sm font-bold text-[#0f1923] mb-4">Issue Details</h3>
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-[#0f1923]">{issue.city}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Ward</span>
                  <span className="font-semibold text-[#0f1923]">{issue.ward}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-[#0f1923]">{issue.comments?.length || 0}</span>
                </div>
            ...
            {/* Share Card */}
            <div className="bg-white rounded-2xl border border-gray-200 p-6">
              <h3 className="text-sm font-bold text-[#0f1923] mb-3">Share this issue</h3>
              <p className="text-xs text-gray-500 mb-3">Help spread awareness by sharing</p>
              <button
                onClick={() => {
